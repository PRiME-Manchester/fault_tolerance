...
/* dumped packets management variables */
uint dump_cnt = 0;
uint dump_msk;
...
// ------------------------------------------------------------------------
// dumped packets management functions
// ------------------------------------------------------------------------
void set_drop_routes (uint core)
{
  uint north_key = 0;
  uint east_key  = 0;
  uint south_key = 0;
  ...
  if (IS_NORTHERNMOST_CORE(core) && !IS_NORTHERNMOST_CHIP(my_x, my_y))
  {
    // expect packets from chip to the north (southernmost core)
    route_from_north = TRUE;
    north_key = ROUTING_KEY(CHIP_TO_NORTH(chipID), SOUTHERNMOST_CORE(core));
  }

  if (IS_SOUTHERNMOST_CORE(core) && !IS_SOUTHERNMOST_CHIP(my_x, my_y))
  {
    // expect packets from chip to the south (northernmost core)
    route_from_south = TRUE;
    south_key = ROUTING_KEY(CHIP_TO_SOUTH(chipID), NORTHERNMOST_CORE(core));
  }

  if (IS_EASTERNMOST_CORE(core) && !IS_EASTERNMOST_CHIP(my_x, my_y))
  {
    // expect packets from chip to the east (westernmost core)
    route_from_east = TRUE;
    east_key = ROUTING_KEY(CHIP_TO_EAST(chipID), WESTERNMOST_CORE(core));
  }

  if (IS_WESTERNMOST_CORE(core) && !IS_WESTERNMOST_CHIP(my_x, my_y))
  {
    // expect packets from chip to the west (easternmost core)
    route_from_west = TRUE;
    west_key = ROUTING_KEY(CHIP_TO_WEST(chipID), EASTERNMOST_CORE(core));
  }

  // set MC routing entries to drop packets from neighbouring chips 
  uint e = rtr_alloc (4);
  if (e == 0)
    rt_error (RTE_ABORT);
...
void take_core_out (uint droute, uint null)
{
  ...
  for (uint i = 0; i < NUM_CPUS; i++)
  {
    if (phys_mask & (1 << i))
    {
      ...
      // construct virtual core mask,
      virt_mask |= 1 << vc;

      // set routing tables to drop packets from neighbouring chips
      set_drop_routes (vc);

      // and clear core temperature
      core_temp[vc - 1] = 0;
    }
  }
...
INT_HANDLER dump_int_han (void)
{
  ...
  if ((type == PKT_TYPE_MC) && (droute != 0))
  {
    ...
    if ((dump_cnt) >= RTR_DUMP_CNT)
    {
      ...
      spin1_schedule_callback (take_core_out, droute, 0, 1);
...
void host_routing_entries_init ()
{
...
void routing_table_init ()
{
  ...
  if (IS_NORTHERNMOST_CORE(coreID))
  {
    if (IS_NORTHERNMOST_CHIP(my_x, my_y))
    {
      ...
      // use border temperature as neighbour's!
...
void update (uint ticks, uint b)
{
  ...
  // check for unattended dumped packets
  // act only on multicast packets
  uint type = rtr[RTR_DHDR] & RTR_TYPE_MASK;

  if (!leadAp && !dumping &&
       (rtr[RTR_STATUS] & (1 << RTR_STATUS_DBIT))
     )
  {
    dump_cnt++;
    if (dump_cnt > (RTR_POLL_CNT * coreID))
    {
...
void c_main()
{
  ...
  {
    // my board-relative address
    board_loc = ((sv->board_addr >> 5) | sv->board_addr) & 63;

    // core temperatures in shared memory (leadAp reports all!)
    ...
    // initialise SDP message buffer to send data to host
    sdp_init ();
